...
npm install -g npm > ~/gcp-ms/backend/logs/npm.txt 2>&1
printf "Completed.\n"

printf "Installing monolith dependencies..."
cd ./monolith
npm install > ~/gcp-ms/backend/logs/monolith.txt 2>&1
printf "Completed.\n"

printf "Installing microservies dependencies..."
cd ../microservices
npm install > ~/gcp-ms/backend/logs/microservices.txt 2>&1
printf "Completed.\n"

printf "Installing React app dependencies..."
cd ../react-app
npm install > ~/gcp-ms/backend/logs/react.txt 2>&1
printf "Completed.\n"

printf "Building React app and placing into sub projects..."
npm run build > ~/gcp-ms/backend/logs/build.txt 2>&1
printf "Completed.\n\n"
